...
export const ImpersonationResultsView: React.FC<ImpersonationResultsViewProps> = ({ result, onReset, onNext }) => {
      ...
      <Card variant="cyber">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-5 flex flex-col items-center justify-center text-center">
            <ScoreRing score={result.score} size={190} strokeWidth={15} label="SCORE" sublabel="IMPERSONATION RESILIENCE" />
          </div>
          ...
          <div className="md:col-span-7 space-y-4 text-left">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="font-mono text-xs text-zinc-500 bg-zinc-50 dark:bg-black px-2.5 py-1 rounded-md border border-zinc-200 dark:border-zinc-800">
                Susceptibility: <strong className="text-green-600 dark:text-green-400">{result.susceptibility_tier}</strong>
              </span>
            </div>

            <h2 className="text-2xl font-bold text-black dark:text-white font-sans">Impersonation Risk Assessment</h2>
            <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{result.summary}</p>

            <div className="pt-2 flex flex-wrap gap-3">
              <Button variant="secondary" size="sm" onClick={onReset} leftIcon={<RefreshCw className="w-4 h-4" />}>Scan Another</Button>
              {onNext && <Button variant="outline" size="sm" onClick={onNext} rightIcon={<ArrowRight className="w-4 h-4" />}>Next: Credentials</Button>}
            </div>
          </div>
        </div>
      </Card>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-black dark:text-white font-sans flex items-center gap-2">
              ...
              Lookalike Vectors ({result.lookalike_variants.length})
            </h3>
            <span className="text-xs font-mono text-zinc-500">DEFENSIVE MONITORING</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 font-mono text-xs">
            {result.lookalike_variants.map((variant, idx) => (
              <div key={idx} className="p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-between hover:border-red-500/40 transition-colors">
                <span className="text-zinc-600 dark:text-zinc-300 truncate max-w-[140px]">@{variant}</span>
                <button type="button" onClick={() => handleCopy(variant)} title="Copy"
                  className="p-1 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-400 hover:text-green-500 transition-colors">
                  {copiedVariant === variant ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-4">
        ...
        {result.findings.length === 0 ? (
          <Card variant="subtle" className="p-8 text-center space-y-3">
            ...
            <h4 className="text-base font-semibold text-green-600 dark:text-green-400 font-sans">High Impersonation Resilience</h4>
            <p className="text-sm text-zinc-500 max-w-md mx-auto">This handle resists spoofing attacks effectively.</p>
          </Card>
        ) : (
          <div className="space-y-4">
