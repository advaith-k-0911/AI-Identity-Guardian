...
export const RecoveryResultsView: React.FC<RecoveryResultsViewProps> = ({ result, onReset, onNext }) => {
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      <Card variant="cyber">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-5 flex flex-col items-center justify-center text-center">
            <ScoreRing score={result.score} size={190} strokeWidth={15} label="SCORE" sublabel="RECOVERY RESILIENCE" />
          </div>
          ...
          <div className="md:col-span-7 space-y-4 text-left">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="font-mono text-xs text-zinc-500 bg-zinc-50 dark:bg-black px-2.5 py-1 rounded-md border border-zinc-200 dark:border-zinc-800">
                Resilience: <strong className="text-green-600 dark:text-green-400">{result.recovery_resilience_tier.split("(")[0]}</strong>
              </span>
            </div>

            <h2 className="text-2xl font-bold text-black dark:text-white font-sans">Recovery Posture</h2>
            ...
            <div className="pt-1 flex flex-wrap gap-2 text-xs font-mono text-zinc-500">
              <span>Backup Codes: <strong className="text-zinc-700 dark:text-zinc-300">{result.backup_codes_status_summary}</strong></span>
            </div>

            <div className="pt-2 flex flex-wrap gap-3">
              <Button variant="secondary" size="sm" onClick={onReset} leftIcon={<RefreshCw className="w-4 h-4" />}>Re-evaluate</Button>
              {onNext && <Button variant="outline" size="sm" onClick={onNext} rightIcon={<ArrowRight className="w-4 h-4" />}>Return to Start</Button>}
            </div>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-4">
        ...
        {result.findings.length === 0 ? (
          <Card variant="subtle" className="p-8 text-center space-y-3">
            ...
            <h4 className="text-base font-semibold text-green-600 dark:text-green-400 font-sans">Hardened Recovery</h4>
            <p className="text-sm text-zinc-500 max-w-md mx-auto">No critical recovery vulnerabilities detected.</p>
          </Card>
        ) : (
          <div className="space-y-4">
      ...
      {result.recommendations.length > 0 && (
        <div className="space-y-3 pt-2">
          <h3 className="text-lg font-bold text-black dark:text-white font-sans flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-green-500" /> Recovery Hardening Plan
          </h3>
